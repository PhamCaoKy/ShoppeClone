  ...
  const purchaseTabLink = purchaseTabs.map((tab) => (
    <Link
      ...
      className={classNames('flex flex-1 items-center justify-center border-b-2 bg-white py-4 text-center', {
        ...
        'border-b-black text-gray-900': status !== tab.status
      })}
    >
      {tab.name}
    </Link>
          ...
          <div key={purchase._id} className='mt-4 rounded-sm border-black/10 bg-white p-6 text-gray-800 shadow-sm'>
            ...
            >
              <div className='flex-shrink-0'>
                <img className='h-20 w-20 object-cover' src={purchase.product.image} alt={purchase.product.name}></img>
              </div>
            ...
            <div className='flex justify-end'>
              ...
              <span className='ml-4 text-xl text-orange'>₫{purchase.product.price * purchase.buy_count}</span>
            </div>
          </div>
        ))}
      </div>
